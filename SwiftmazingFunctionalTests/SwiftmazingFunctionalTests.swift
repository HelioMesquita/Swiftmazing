...
class SwiftmazingFunctionalTests: KIFTestCase {
  ...
  func testTapOnFeedRepositoryCellItShowsDetail() {
    ...
    tester().waitForView(withAccessibilityLabel: repository.description)
    tester().waitForView(
      withAccessibilityLabel: " - \(repository.stars.kiloFormat) \(Text.stars.value)")
    tester().waitForView(
      withAccessibilityLabel: " - \(repository.issues.kiloFormat) \(Text.issues.value) ")
    tester().waitForView(
      withAccessibilityLabel: " - \(repository.forks.kiloFormat) \(Text.forks.value) ")
    tester().waitForView(
      withAccessibilityLabel: " - \(Text.lastUpdate.value) \(repository.lastUpdate.monthDayYear)")
    tester().navigateToFeed()
  }
